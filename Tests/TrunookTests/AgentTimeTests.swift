...
struct AgentTimeTests {
    ...
    /// Описание поля времени учит модель писать день словом. Пример,
    /// который разборщик не понимает, хуже, чем никакого: модель старательно
    /// повторит его, а человек получит отказ инструмента вместо встречи.
    @Test("Примеры из описания разбираются")
    func примерыИзОписанияРазбираются() throws {
        let ru = Locale(identifier: "ru_RU")
        // 11 сентября 2026 — пятница, 14:07.
        let ждём: [(String, Int, Int, Int)] = [
            ("завтра 15:00", 12, 15, 0),
            ("послезавтра 19:00", 13, 19, 0),
            ("понедельник 12:00", 14, 12, 0),
            ("пятница 18:30", 11, 18, 30),
            ("завтра 09:00", 12, 9, 0),
        ]
        for (текст, день, час, минута) in ждём {
            let moment = try #require(
                AgentTime.parse(текст, now: now, calendar: calendar, locale: ru),
                "не разобрано: \(текст)"
            )
            #expect(parts(moment.date).day == день, "не тот день у «\(текст)»")
            #expect(parts(moment.date).hour == час, "не тот час у «\(текст)»")
            #expect(parts(moment.date).minute == минута, "не та минута у «\(текст)»")
        }
    }
