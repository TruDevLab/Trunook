...
/// Разговор голосом идёт кругами: сказали, услышали, уточнили.
///
/// Проверять это микрофоном из сессии нечем, поэтому правило вынесено
/// из захода отдельно — им и решается, гаснет вырез после ответа
/// или слушает дальше.
@Suite("Круги голосового разговора")
struct VoiceTurnTests {
    /// Ради этого всё и затевалось: уточнить сказанное, не поднимая руки
    /// к жесту.
    @Test("Дочитав ответ, вырез слушает дальше")
    func послеОтветаСлушаем() {
        #expect(VoiceSession.awaitsReply(keepsListening: true, hasPending: false, wasSpeaking: true))
    }

    /// На карточку отвечают нажатием, а не голосом. Слушать в это время
    /// значило бы обещать голосу то, чего он не может.
    @Test("Под карточкой подтверждения не слушаем")
    func подКарточкойМолчим() {
        #expect(!VoiceSession.awaitsReply(keepsListening: true, hasPending: true, wasSpeaking: true))
    }

    @Test("Выключено в настройках — заход гаснет, как прежде")
    func выключенноеНеСлушает() {
        #expect(!VoiceSession.awaitsReply(keepsListening: false, hasPending: false, wasSpeaking: true))
    }

    /// Сюда можно попасть от синтезатора, дочитывавшего последнюю фразу,
    /// когда всё остальное уже погасло. Оживлять погашенный заход нельзя:
    /// микрофон открылся бы сам собой.
    @Test("Оборванный заход не оживает")
    func оборванныйНеОживает() {
        #expect(!VoiceSession.awaitsReply(keepsListening: true, hasPending: false, wasSpeaking: false))
    }
}
