...
struct SettingsView: View {
    ...
    private func chatModelChoices(keeping current: String) -> [String] {
        ...
        Group {
            if !settings.ollamaEnabled {
                section(t("Нужна модель"), icon: "exclamationmark.triangle") {
                    hint(t("Сводки и слежку собирает модель — включите её в разделе «ИИ»."))
                }
            }
            digestSection
            siteWatchSection
    ...
    private var digestSection: some View {
        section(t("Сводка новостей"), icon: "newspaper") {
            VStack(alignment: .leading, spacing: 4) {
                Toggle(t("Собирать сводку новостей"), isOn: Binding(
    ...
    private var topicSuggestions: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 8) {
                Button {
    ...
    private var digestStatus: String {
    ...
    private var siteWatchSection: some View {
        section(t("Слежка за сайтами"), icon: "binoculars") {
            VStack(alignment: .leading, spacing: 4) {
                Toggle(t("Следить за сайтами"), isOn: Binding(
    ...
    private func siteWatchRow(_ watch: SiteWatch) -> some View {
    ...
    private func siteStatus(_ watch: SiteWatch, _ state: WatchState) -> String {
    ...
    private static func feedStamp(_ date: Date) -> String {
        let formatter = DateFormatter()
        formatter.locale = Localization.shared.resolved.locale
