...
enum RecommendedModel {
    ...
    ///
    /// Выбрана пробой, а не по отзывам, и главным доводом оказалась
    /// **скорость устного ответа**. Требований к модели здесь четыре сразу:
    /// звать инструменты, говорить по-русски, помещаться в память рядом
    /// с работой человека и отвечать голосом быстрее, чем человек успеет
    /// решить, что приложение зависло.
    ///
    /// Прежней была `gemma4:12b` — и по вызовам инструментов она безупречна,
    /// но на устный вопрос уходила от двадцати семи до пятидесяти восьми
    /// секунд: почти всё это время она думает, и перед ответом вырез молча
    /// светится. `qwen3:8b` отвечает за шесть-двенадцать, занимает 5,2 ГБ
    /// вместо 7,6 и все пробы вызовов проходит так же чисто.
    ///
    /// `gpt-oss:20b` быстра не меньше и по-русски пишет чище всех, но весит
    /// 13,8 ГБ — вдвое с лишним больше; на машине с 24 ГБ это заметная доля.
    /// `qwen3:4b` дешевле всех, но путается в заведении встречи.
    ///
    /// Подробности пробы — в `DEVELOPMENT.md`, «Какой моделью отвечать».
    static let chat = "qwen3:8b"
