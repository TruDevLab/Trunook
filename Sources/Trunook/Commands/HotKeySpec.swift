...
struct HotKeySpec: Codable, Equatable, Hashable {
    ...
    /// ⌃⌥C — захватить выделенное и открыть разговор с моделью.
    ///
    /// Тем же сочетанием раньше открывалось меню быстрых команд, и буква
    /// оставлена нарочно: команды никуда не делись, они переехали в панель
    /// разговора, — а привычка нажимать ⌃⌥C, чтобы что-то сделать
    /// с выделенным, ровно та же. «C» читается и как «команды», и как copy:
    /// захват выделенного — это и есть копирование.
    ///
    /// До того было ⌥⌘Space, и это оказалось системным сочетанием: им macOS
    /// открывает окно поиска Finder. Приложение регистрировало его поверх,
    /// и что сработает — зависело от того, кто успел первым.
    static let assistant = HotKeySpec(keyCode: UInt32(kVK_ANSI_C), modifiers: own)

    /// ⌃⌥1 … ⌃⌥9 — сочетание по умолчанию для команды, стоящей на этом месте
    /// при первом запуске.
    ///
    /// Дальше сочетание живёт при самой команде, а не при её месте в списке:
    /// переставили — уехало вместе с ней. Иначе перестановка молча меняла бы
    /// то, что делает знакомая клавиша.
