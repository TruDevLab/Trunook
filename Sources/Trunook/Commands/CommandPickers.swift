...
final class ModelList: ObservableObject {
    ...
    /// Запомнить отказ сервера.
    ///
    /// Ollama отвечает «does not support tools» отказом на сам запрос,
    /// и это последний источник правды: у необновлённого списка умений
    /// ответа ещё нет, а спросить `/api/show` заранее успевает не всегда —
    /// команда по горячей клавише уходит модели сразу.
    ///
    /// Запись сюда значит, что второй раз инструменты этой модели
    /// не достанутся вовсе.
    func noteNoTools(_ name: String) {
        guard !name.isEmpty else { return }
        var able = known[name] ?? []
        able.remove("tools")
        // Пустой набор читается как «не знаю», поэтому кладём хоть что-то:
        // ответ «умений нет» и ответ «не спрашивали» должны различаться.
        able.insert("chat")
        known[name] = able
        tooling.remove(name)
        DebugLog.write("модели: \(name) не умеет инструменты — запомнили")
    }
