...
struct QuickCommand: Codable, Equatable, Identifiable {
    ...
    ///
    /// **Имя обязательно, и это не украшение.** Разбор набора выбрасывает
    /// записи, у которых пусты и имя, и содержимое, — так уходят пустышки
    /// старого формата. Заготовка без имени попадала под то же правило
    /// и исчезала в тот же миг, в который заводилась: кнопка «Добавить
    /// команду» выглядела не работающей вовсе.
    ///
    /// В список выреза заготовка при этом не попадёт — там нужен ещё
    /// и заполненный запрос (`isConfigured`).
    static func blank(id: Int) -> QuickCommand {
        QuickCommand(
            id: id,
            title: t("Новая команда"),
            kind: .ollama,
            payload: "",
            symbol: "",
            isEnabled: true,
