...
final class ReleaseNotesService: NSObject, ObservableObject {
    ...
    /// Страницу открыли — показываем свежий выпуск.
    ///
    /// Отдельно от `load()`, и это не украшательство. Выбор страницы живёт
    /// в службе, а служба — на всё приложение: закрыв описание на выпуске
    /// 0.9.0, человек в следующий раз открывал «Что нового» и снова видел
    /// 0.9.0. Прежний выбор при этом не сбрасывался ничем — ни новым
    /// выпуском, ни закрытием окна.
    ///
    /// «Что нового» — это вопрос, а не место, и отвечать на него надо
    /// каждый раз заново. Возврат к прежней странице оставлен там, где он
    /// уместен: `load()` из кнопки «Попробовать снова» ничего не сбрасывает.
    func present() {
        openedByHand = false
        if let newest = notes.first {
            selection = .release(tag: newest.tag)
        } else {
            // Ответа ещё нет — вернём выбор к README, и приехавший ответ
            // сам переставит его на свежий выпуск (см. `apply`).
            selection = .readme
        }
        load()
    }
    ...
    private func apply(_ next: State) {
        ...
        //
        // Переставляем с любой страницы, а не только с README: приехавший
        // ответ бывает свежее той, что показана из кэша. Человека, выбравшего
        // страницу руками, не трогаем — на то и `openedByHand`.
        if case let .ready(notes) = next, let newest = notes.first, !openedByHand {
            selection = .release(tag: newest.tag)
        }
    }
