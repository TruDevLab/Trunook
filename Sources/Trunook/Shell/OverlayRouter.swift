...
///
/// Накладки занимают одно место: история буфера, разговор с моделью,
...
final class OverlayRouter {
    ...
    /// Накладку убрал ушедший курсор — не крестик, не клавиша, не открытие
    /// соседней. Отличать этот случай нужно потому, что он единственный
    /// означает «человек ушёл от выреза совсем», а не «закончил с этой
    /// панелью и остался».
    ...
    func updateHover(at location: CGPoint) {
        ...
        // Пока шкалу таймера тянут, панель не закрывается ничем.
        //
        // Пятнадцать минут по шкале — это почти двести точек хода, и рука
        // за них выходит за края панели раньше, чем добирается до нужного
        // деления. Панель, закрывшаяся посреди жеста, выглядит поломкой:
        // человек ведёт руку, а всё пропадает.
        //
        // Правило накладки при этом остаётся прежним — отпустил, ушёл
        // курсором, закрылось.
        //
        // Отпускание проверяется состоянием кнопки мыши, а не только концом
        // жеста: `DragGesture` о своём конце не сообщает, если кнопку
        // отпустили за пределами окна, — и накладка осталась бы открытой
        // навсегда. Опрос здесь и так идёт десять раз в секунду.
        if TimerDialGrip.shared.isHeld {
            guard NSEvent.pressedMouseButtons == 0 else { return }
            TimerDialGrip.shared.end()
        }
    ...
    static func name(of overlay: NotchState.Overlay) -> String {
        ...
        case .windowSnap: return "раскладка окна"
        }
    }
}
