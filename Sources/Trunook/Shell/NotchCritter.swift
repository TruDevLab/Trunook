...
/// сердится и грозит кулаком, ловит мышь, загорает под
/// зонтом, гоняется за птичкой и следит за курсором.
...
final class NotchCritter: ObservableObject {
    enum Act: String, CaseIterable {
        ...
        /// на затяжках, дым.
        case smoke
        /// Сбоку выбегает мышь, котик гонится за ней, ловит и уносит
        /// в зубах в чёлку.
        case mouse
        /// Чёлка светит солнцем: котик ставит зонт, лежит под ним со смузи,
        /// потом забирает зонт и уходит.
        case beach
        /// Из чёлки вылетает птичка, котик гоняется за ней и прыгает —
        /// она улетает обратно в чёлку, а он уходит следом.
        case bird
        /// Котик садится под чёлкой и водит глазами за курсором.
        case watch
        ...
        var duration: TimeInterval {
            ...
            case .cool: return 7
            case .smoke: return 8
            case .mouse: return 10.4
            case .beach: return 12.0
            case .bird: return 10.4
            case .watch: return 9.0
        ...
        var followsCursor: Bool {
            self == .eyes || self == .paw || self == .hunt || self == .watch
        }
