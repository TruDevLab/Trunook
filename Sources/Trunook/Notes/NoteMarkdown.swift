...
enum NoteMarkdown {
    ...
    private static func line(in text: NSAttributedString, range: NSRange) -> String {
        ...
        if isHeading { return "## " + result }
        // Пункт списка уходит разметкой `- [ ]`. Зачёркивание отмеченного —
        // оформление самой галочки, в разметке ему места нет.
        return Checklist.markdown(fromDisplay: result)
    }
